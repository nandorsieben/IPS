...
# spanning set
b={k:[model.NewIntVar(-1,1,f'b_{k}_{i}') for i in range(n)] for k in range(1,n)}
for k in b:
  for i in range(n):
    b1=reiff(c[i]== k,c[i]!= k)
    b2=reiff(c[i]==-k,c[i]!=-k)
    model.Add(b[k][i]== 1).OnlyEnforceIf(b1)
    model.Add(b[k][i]==-1).OnlyEnforceIf(b2)
    model.Add(b[k][i]== 0).OnlyEnforceIf(b1.Not(),b2.Not())
...
for i in range(1,n):
  ...
  model.AddBoolOr([reiff(c[i]>=0,c[i]<0)]+[reiff(c[j]==-c[i],c[j]!=-c[i]) for j in range(i)])
# M-invariance
for i in range(n):
  model.AddBoolAnd([reiff(w[k][i]==0,w[k][i]!=0) for k in b]).OnlyEnforceIf(reiff(c[i]==0,c[i]!=0))
  for j in range(i):
